...
@test "${TEST_LABEL} > Testing CLANG" {
    ...
    if [[ $NAME == "Darwin" ]]; then
        ...
        install-package build-essential BYPASS_DRYRUN 1>/dev/null # ubuntu 18 build-essential will be high enough, 16 won't and has a version < 7
        run bash -c "printf \"y\n%.0s\" {1..100} | ./$SCRIPT_LOCATION"
        ...
        ## CLANG
        uninstall-package build-essential BYPASS_DRYRUN 1>/dev/null
